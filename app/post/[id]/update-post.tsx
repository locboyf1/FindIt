...
export default function UpdatePostScreen() {
    ...
    const fetchPost = async () => {
        setIsLoading(true);
        try {
            const fetch_post: any = await getPostById(id as string);
            if (!fetch_post) {
            ...
            setType(fetch_post.type);
            setImage(fetch_post.image);
            setSelectedProvince({
                id: fetch_post.provinceId,
                name: fetch_post.provinceName
            });
            setSelectedWard({
        ...
        <SafeAreaView className="flex-1 bg-white">
            ...
            >
                <View className="flex-row justify-between items-center px-5 py-[15px] border-b border-[#eee]">
                    ...
                    <Text className="text-[18px] font-bold text-[#333]">Sửa tin</Text>
